...
fixed4 frag(VS_OUT inp) : COLOR {
	...
	#if defined(FERR2DT_LIGHTMAP) || MAX_LIGHTS > 0
	color.rgb *= light;
	#endif
	...
	#if defined(FERR2DT_BLACKWNWHITE)
	half3 c = color.rgb ;
	float gray = saturate(color.r * 0.299 + color.g * 0.587 + color.b * 0.114) ;	
	half3 b = half3(gray, gray, gray) ;
	float dist = lerp(0,1,light);
	color.rgb =  lerp(b, c, dist);
	#endif
